...
public class TypeChecker extends MiniJavaBaseListener {
    ...
    public void exitExpBinOp(MiniJavaParser.ExpBinOpContext ctx) {
        ...
        switch (op) {
            ...
            case ">":
            case "<":
                // Add the custom error message for the < and > operators here
            if (!lhs.isInt()) {
                this.error(ctx, "Expected int as 1st argument to " + op + "; actual type: " + lhs);
            }
            if (!rhs.isInt()) {
                this.error(ctx, "Expected int as 2nd argument to " + op + "; actual type: " + rhs);
            }
            break;
